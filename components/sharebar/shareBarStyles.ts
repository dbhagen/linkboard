...
export const ShareBarContainer = styled.button`
  ...
  flex-direction: row;
  background-color: rgba(${({ theme }) => theme.colors.background}, 0.5);
  ...
  &:hover {
    background-color: rgba(${({ theme }) => theme.colors.background}, 0.7);
  }
`;
...
export const ShareLinksContainer = styled.div<ShareLinksContainerProps>`
  ...
  width: 0;
  padding-inline: 0;
  animation: ${({ $open }) => ($open ? slideIn : slideOut)} 0.5s ease-out
    forwards;
...
export const ShareDialog = styled.dialog`
  ...
  &::backdrop {
    background-color: rgba(${({ theme }) => theme.colors.background}, 0.8);
